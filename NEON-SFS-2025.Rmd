...
Sample types (e.g., surber, hand corer, kicknet) are strongly associated with 
...
Below, for CUPE and GUIL, we will use a combination of samplerType and 
habitatType.
...
table_sample_info <- inv_clean %>%
  select(sampleID, domainID, siteID, namedLocation, 
         collectDate.x, eventID, boutNumber, 
         habitatType, samplerType, benthicArea) %>%
  distinct()

# first, remove singletons and doubletons to create an occurrence summary table
taxa_occurrence_summary <- inv_clean %>%
  select(sampleID, acceptedTaxonID, scientificName) %>%
  distinct() %>%
  group_by(scientificName) %>%
  summarize(occurrences = n())


# filter out rare taxa that are only observed 1 or 2 times
...
inv_richness_summ <- inv_clean %>%
  filter(scientificName %in%
             taxa_list_cleaned$scientificName) 


# select only site by species density info and remove duplicate records
inv_richness_summ_long <- inv_richness_summ %>%
  select(sampleID, acceptedTaxonID, scientificName, abun_M2) %>%
  distinct() %>%
  filter(!is.na(abun_M2))


# pivot to wide format, sum multiple counts per sampleID
inv_richness_summ_wide <- inv_richness_summ_long %>%
  ...
  round() #round to integer so that vegan functions will run


# check col and row sums -- mins should all be > 0 for further analysis in vegan
colSums(inv_richness_summ_wide) %>% min()
rowSums(inv_richness_summ_wide) %>% min()


# use the vegan package to calculate diversity indices abd merge into one table
# with metadata
inv_richness <- as.data.frame(specnumber(inv_richness_summ_wide)) %>% #more sensitive to rare taxa
  rename(richness = `specnumber(inv_richness_summ_wide)`) 

inv_richness_stats <- estimateR(inv_richness_summ_wide)

inv_evenness <- as.data.frame(diversity(inv_richness_summ_wide) / log(specnumber(inv_richness_summ_wide))) %>% 
  rename(evenness = `diversity(inv_richness_summ_wide)/log(specnumber(inv_richness_summ_wide))`)

inv_shannon <- as.data.frame(diversity(inv_richness_summ_wide, index = "shannon")) %>% 
  rename(shannon = `diversity(inv_richness_summ_wide, index = "shannon")`)

inv_simpson <- as.data.frame(diversity(inv_richness_summ_wide, index = "simpson")) %>% #less sensitive to rare taxa
  rename(simpson = `diversity(inv_richness_summ_wide, index = "simpson")`)

inv_diversity_indices <- bind_cols(inv_richness, inv_evenness, inv_shannon, inv_simpson)
rm(inv_richness, inv_evenness, inv_shannon, inv_simpson)

inv_diversity_indices <- left_join(rownames_to_column(inv_diversity_indices), table_sample_info, by = c("rowname" = "sampleID")) %>%
  rename(sampleID = rowname)

inv_diversity_indices_long <- inv_diversity_indices%>%
  tidyr::pivot_longer(c(richness,evenness,shannon,simpson),
                      names_to = "indexName",
                      values_to = "indexValue")

inv_diversity_summ <- inv_diversity_indices_long%>%
  dplyr::rename(collectDate=collectDate.x)%>%
...
# add in abundance
names(inv_abundance_summ)
names(inv_diversity_summ)

inv_summ_allIndices <- inv_abundance_summ%>%
  rename(mean=abun_M2_sum_mean,
         sd=abun_M2_sum_sd,
         se=abun_M2_sum_se)%>%
  mutate(n=NA,
         indexName="abundance")%>%
  select(names(inv_diversity_summ))%>%
  rbind(inv_diversity_summ)


# produce plot to show trends within and across sites
inv_diversity_plot <- inv_summ_allIndices%>%
