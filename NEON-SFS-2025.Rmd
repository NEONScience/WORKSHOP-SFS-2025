...
First, we remove the records that were collected outside of normal sampling 
bouts as a grab sample, in cases where the NEON field ecologists saw interesting
...
habitat (i.e., riffle, run, pool) and substrata. Are some NEON sites, like D04 
CUPE, a surber sampler is used in two habitat types (riffle and run) because 
all habitats have the same cobble substrata. Data users should look at the data
to determine how they want to discriminate between sampler or habitat type.
...
inv_abundance_summ <- inv_clean%>%
  ...
  dplyr::group_by(siteID,collectDate,eventID,samplerType,habitatType,boutNumber)%>%
